...
public class PersistentConnectionHandler implements ConnectionHandler {
  ...
  public void handle() {
    ...
    log.info(
        "Connection received, handling connection from remote address {}", remoteSocketAddress);
  ...
  private void closeConnection(SocketAddress remoteSocketAddress) {
    ...
    try {
      ...
      log.error(
          "Error while closing the connection from remote address {}", remoteSocketAddress, e);
    }
  }
  ...
  private void executeCommand(Socket socket, String command, Tokens tokens) throws IOException {
    try {
      final CommandExecutor commandExecutor =
          commandExecutorFactory.getCommandExecutor(tokens.commandType());
