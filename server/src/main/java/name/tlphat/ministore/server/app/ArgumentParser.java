...
public class ArgumentParser {
    ...
    private final Set<String> supportedArguments = Arrays.stream(ArgumentKey.values())
                                                         .map(ArgumentKey::getKey)
                                                         .collect(Collectors.toSet());

    /**
     * Parse the program argument into a collection of key value pairs, packaged in a {@link ProgramArguments} object.
     *
    ...
    public ProgramArguments parse(String[] args) {
        final Map<String, String> argumentMap = Arrays.stream(args)
                                                      .map(ArgumentParser::splitArgumentValue)
                                                      .map(this::validateParts)
                                                      .collect(Collectors.toMap(
                                                          parts -> parts.get(0), parts -> parts.get(1)));
        ...
        final Optional<Long> snapshotWriteDuration =
            Optional.ofNullable(argumentMap.get(ArgumentKey.SNAPSHOT_WRITE_DURATION_KEY.getKey())).map(Long::parseLong);
     ...
     * <p>
     * This method only split the at the first occurrence of "=",
     * so the result will be an array with at mose 2 elements.
     * <p>
     * If there is no "=" in the input String, this method returns a List with only 1 element,
    ...
    private static List<String> splitArgumentValue(String arg) {
        ...
        return Arrays.stream(arg.split(EQUALS_SIGN, 2))
                     .map(String::trim)
                     .toList();
    }
